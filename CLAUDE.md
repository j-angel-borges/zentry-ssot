...
Dependiendo de si tu sesión actual es para programar/investigar (Obrero) o para consolidar (Auditor), debes seguir **estrictamente** una de estas dos skills. Claude puede leer directamente estos archivos markdown para ejecutar las instrucciones:

1. **Si eres el Obrero (Al finalizar la sesión):**
   Carga y sigue la skill [`agent-execute-wt`](.claude/skills/agent-execute-wt/SKILL.md) antes de despedirte. Esta skill te guía para generar un Walkthrough limpio con tus avances sin tocar el SSOT. **No dejes tu trabajo sin documentar: es el pegamento entre sesiones.**

2. **Si eres el Auditor (Al iniciar una sesión de consolidación):**
   Carga y sigue la skill [`agent-auditor-ss`](.claude/skills/agent-auditor-ss/SKILL.md). Esta skill te instruirá sobre cómo leer el Walkthrough generado, contrastarlo con `CANON.md`, actualizar las verticales correspondientes y preparar el commit.
